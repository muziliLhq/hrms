...
function listHandler(list) {
    ...
    for (var index = 0; index < list.length; index++) {
        ...
        var tr = $('<tr></tr>');
        // 定义列
        var ck = $('<th scope="row"><input type="checkbox">')
...
function query() {
    ...
    // 2.请求服务器
    $.getJSON(basePath + '/notice/list', {title: title, createDate: createDate, content: content}, function (data) {
        dataHandler(data);
    });
}
...
function lookItem(that) {
    ...
    /* $(parent.document).find('.page-wrapper')是从子页面hrms_user.ftl中
      ...
      保证局部与整体良好的分离特点。*/
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/notice/look/" + noticeId + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function updateItem(that) {
    ...
    // 加载更新页面
    // 更新页面是请求服务器得到的
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/notice/update?id=" + noticeId + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function deleteItem(that) {
    ...
    $('#userModalCenter .modal-footer .btn-primary').unbind().click(function () {
        ...
        var noticeId = getIdOfRow(that);
        // 请求服务器删除该记录
        $.getJSON(basePath + '/notice/del/' + noticeId, function (data) {
            alert(data.message);
            // 刷新列表
            $.getJSON(basePath + '/notice/list', function (data) {
                dataHandler(data);
            });
        });
    });
...
function batchDeleteItems() {
    ...
    $('#userModalCenter .modal-footer .btn-primary').unbind().click(function () {
        ...
        $(':checkbox:checked').each(function () {
            noticeIds.push(getIdOfRow($(this)));
        });
        // 请求服务器删除该记录
        $.getJSON(basePath + '/notice/batchDelete', {ids: noticeIds}, function (data) {
            alert(data.message);
            // 刷新列表
            $.getJSON(basePath + '/notice/list', function (data) {
                dataHandler(data);
