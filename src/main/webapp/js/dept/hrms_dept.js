...
function listHandler(list) {
    ...
    for (var index = 0; index < list.length; index++) {
            ...
            var deptId = $('<td>' + list[index].id + '</td>');
            ...
            // 组行和列并附加到tbody
            $('tbody').append(tr.append(ck).append(th).append(deptId).append(name).append(remark).append(operation));
        }
}
...
function updateItem(that) {
    // 获取行id
    var deptId = getIdOfRow(that);
    // 加载更新页面，更新页面是请求服务器得到的
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/dept/update?id=" + deptId + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function lookItem(that) {
    // 获取行的id
    var deptId = getIdOfRow(that);
    // 加载查看页面，查看页面是请求服务器得到的
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/dept/look/" + deptId + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
/**
 * 删除
 * @param that
...
function deleteItem(that) {
    ...
    $('#userModalCenter .modal-footer .btn-primary').unbind().click(function () {
        ...
        var deptId = getIdOfRow(that);
        // 请求服务器删除行
        $.getJSON(basePath + '/dept/del/' + deptId, function (data) {
            alert(data.message);

            // 刷新列表
            $.getJSON(basePath + '/dept/list', function (data) {
                dataHandler(data);
            })
        })
...
function batchDeleteItems() {
    ...
    $('#userModalCenter .modal-footer .btn-primary').unbind().click(function () {
        ...
        $(':checkbox:checked').each(function () {
            deptIds.push(getIdOfRow($(this)));
        });
        // 请求服务器删除该记录
        $.getJSON(basePath + '/dept/batchDelete' , {ids: deptIds}, function (data) {
            alert(data.message);
            // 刷新列表
            $.getJSON(basePath + '/dept/list', function (data) {
                dataHandler(data);
